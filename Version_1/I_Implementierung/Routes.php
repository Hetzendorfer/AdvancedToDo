...
Route::set("projects", function(){
    ...
    switch($_SERVER["REQUEST_URI"]){
        case "POST":
            ...
            if(isset($arr->project_name, $arr->employee_id, $arr->project_description, $arr->project_status, $arr->project_activ)){
                if(Project::createProject($arr->project_name, $arr->employee_id, $arr->project_description, $arr->project_status, $arr->project_activ)){
                    http_response_code(200);
                    echo json_encode(array("success" => "true", "message" => "Creation was successfull"));
                }
                else{
...
Route::set("projects/departments", function(){
    ...
    switch($_SERVER["REQUEST_URI"]){
        case "POST":
            ...
            if(isset($arr->project_id, $arr->department_id)){
                if(Project::addDepartment($arr->project_id, $arr->department_id)){
                    http_response_code(200);
                    echo json_encode(array("success" => "true", "message" => "Creation was successfull"));
                }
                else{
        ...
        case "DELETE":
            ...
            if(isset($arr->project_id, $arr->department_id)){
                if(Project::deleteDepartment($arr->project_id, $arr->department_id)){
                    http_response_code(200);
                    echo json_encode(array("success" => "true", "message" => "Creation was successfull"));
                }
                else{
        ...
        case "GET":
            if(isset($_GET["id"])){
                $info = Project::getDepartments($_GET["id"]);
                http_response_code(200);
                echo json_encode($info);
            }
            break;
