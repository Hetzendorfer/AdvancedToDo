...
class Authentication{
    ...
    public static function Login($email, $password){
        ...
        $query = "SELECT login_daten.mitarbeiter_id AS ID, login_daten.is_management AS IS_MANAGEMENT FROM login_daten WHERE login_daten.email = :email AND login_daten.password = :password;";
        $stmt = $conn->prepare($query);
        ...
        if($stmt->rowCount() > 0){
            ...
            if($stmt->execute()){
                ...
                if($is_management != NULL){
                    setcookie("is_management", "true");
                    $_SERVER["is_management"] = true;
                }
                else{
                    setcookie("is_management", "false");
                    $_SERVER["is_management"] = false;
                }
                $_SERVER["token"] = $token;
                $_SERVER["user_id"] = $id;
                return $token;
